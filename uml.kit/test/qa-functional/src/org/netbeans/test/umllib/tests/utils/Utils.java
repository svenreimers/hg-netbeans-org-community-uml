...
public class Utils {
   ...
   {
       setDefaultPreferences("No");
   }
   public static void setDefaultPreferences(String promptToSave)
   {
        ...
        pr.setValue(1);
        if(!pr.getValue().equalsIgnoreCase(promptToSave))pr.setValue(0);
        new JButtonOperator(op,"Close").push();
   }
   public static void commonTestsSetup(String workdir,String prName)
   {
       commonTestsSetup(workdir,prName,"No");
   }
   public static void commonTestsSetup(String workdir,String prName,String promptToSave)
   {
            //setDefaultPreferences(promptToSave);
            ...
            java.awt.Rectangle pth=catTree.getPathBounds(catTree.findPath(LabelsAndTitles.UML_PROJECTS_CATEGORY));
            catTree.moveMouse(pth.x+pth.width/3, pth.y+pth.height/2);
            catTree.selectPath(catTree.findPath(LabelsAndTitles.UML_PROJECTS_CATEGORY));
            catTree.waitSelected(catTree.findPath(LabelsAndTitles.UML_PROJECTS_CATEGORY));
            new EventTool().waitNoEvent(500);
            newWizardOper.selectProject(LabelsAndTitles.ANALYSIS_UML_PROJECT_LABEL);
            newWizardOper.next();
