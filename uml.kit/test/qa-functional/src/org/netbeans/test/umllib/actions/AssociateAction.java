...
public class AssociateAction extends ActionNoBlock{
    ...
    private static final String popupPath = "Associate With...";
    
    public AssociateAction() {
        super(null, popupPath);
    }

    public void performPopup(DiagramElementOperator elem){
        if (popupPath==null)
            throw new UnsupportedOperationException(getClass().toString()+" does not define popup path");
        new EventTool().waitNoEvent(500);
        elem.clickForPopup();
        JPopupMenuOperator popup = new JPopupMenuOperator(elem);
