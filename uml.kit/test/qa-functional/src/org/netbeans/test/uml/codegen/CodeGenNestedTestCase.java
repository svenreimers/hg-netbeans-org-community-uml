 ...
 * 
 * Created on Jun 5, 2007, 4:58:55 PM
 * 
...
public class CodeGenNestedTestCase extends CodeGenerationTestCase {
    ...
    IPackageElem parentPackage = new PackageElem("links");
...
/** Creates a new instance of GenCodeLinksTestCase 
    ...
    public void testClassProjectGeneration(){     
        generateProject(parentPackage.getName() + "|" + pack.getName());
    }
    
    
    public IClassElem  getOuterClass(){
        IClassElem elem = new ClassElem("Outer", pack );
        elem.getNestedElemList().add(getInnerClass());
        return elem;
    }
    
    public IClassElem  getInnerClass(){
        IClassElem elem = new ClassElem("Inner", pack );
        return elem;
    }
    
    //  class Outer { class Inner {} }
    
    public void testNestedClassTextClasses(){
        IJavaElem elem = getOuterClass();
        assertVerification(new TextElementVerifier(getEditorOperator(elem), elem));
    }
    
    public void testNestedClassReflectionClasses(){
        assertVerification(new ReflectionElementVerifier(getOuterClass(), getJavaProjectName()));
    }
