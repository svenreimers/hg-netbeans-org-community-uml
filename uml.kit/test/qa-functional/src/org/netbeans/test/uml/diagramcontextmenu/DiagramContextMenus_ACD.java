...
public class DiagramContextMenus_ACD extends UMLTestCase {
    ...
    public DiagramContextMenus_ACD(String name) {
        super(name);
    }
    
    public static NbTestSuite suite() {
        NbTestSuite suite = new NbTestSuite(org.netbeans.test.uml.diagramcontextmenu.DiagramContextMenus_ACD.class);
        return suite;
    }
 ...
 * @usecase Check context menu of activity diagram
 */
 ...
 */
    public void testItemLayoutHierarchical(){
        ...
        JMenuItemOperator item = DCMUtils.checkDiagramPopupMenuItem(diagram, itemName);
        if(item==null){
            fail(78330, "Diagram popum menu item '" + itemName + "' not found");
        }else if(!item.isEnabled()){
            fail("Diagram popum menu item '" + itemName + "' is disabled but should not be");
        }
    }
...
//------------------------------------------------------------------------------
    ...
    public void setUp() throws FileNotFoundException{
        ...
        diagram = DCMUtils.openDiagram(prName, acdName, NewDiagramWizardOperator.ACTIVITY_DIAGRAM, workDir);
        if (diagram == null){
