...
public class PlaceMultipleLinks extends UMLTestCase {
    ...
    private static String prName= "componentDiagramProjectML";
   ...
   //--
    ...
    public PlaceMultipleLinks(String name) {
        super(name);
    }
     public static NbTestSuite suite() {
        NbTestSuite suite = new NbTestSuite(org.netbeans.test.uml.componentdiagram.PlaceMultipleLinks.class);
        return suite;
    }
  ...
  public void testCreateLink1() {
        ...
        testLinkPlace(workPkg1,componentDiagramName1, fromType1, toType1,element1,elementName1,elementType1,treeNode1,eIO1,false);
        testLinkPlace(workPkg1,componentDiagramName1, fromType1, toType1,element1,elementName1,elementType1,treeNode1,eIO1,true);
    }
 ...
 /* public void testCreateLink3() {
  ...
  public void testCreateLink6() {
        lastTestCase=getCurrentTestMethodName();
        testLinkPlace(workPkg6,componentDiagramName6, fromType6, toType6,element6,elementName6,elementType6,treeNode6,eIO6,false);
        testLinkPlace(workPkg6,componentDiagramName6, fromType6, toType6,element6,elementName6,elementType6,treeNode6,eIO6,true);
    }
  public void testCreateLink7() {
        lastTestCase=getCurrentTestMethodName();
        testLinkPlace(workPkg7,componentDiagramName7, fromType7, toType7,element7,elementName7,elementType7,treeNode7,eIO7,false);
        testLinkPlace(workPkg7,componentDiagramName7, fromType7, toType7,element7,elementName7,elementType7,treeNode7,eIO7,true);
    }
  public void testCreateLink8() {
        lastTestCase=getCurrentTestMethodName();
        testLinkPlace(workPkg8,componentDiagramName8, fromType8, toType8,element8,elementName8,elementType8,treeNode8,eIO8,false);
        testLinkPlace(workPkg8,componentDiagramName8, fromType8, toType8,element8,elementName8,elementType8,treeNode8,eIO8,true);
    }
  public void testCreateLink9() {
        lastTestCase=getCurrentTestMethodName();
        testLinkPlace(workPkg9,componentDiagramName9, fromType9, toType9,element9,elementName9,elementType9,treeNode1,eIO9,false);
        testLinkPlace(workPkg9,componentDiagramName9, fromType9, toType9,element9,elementName9,elementType9,treeNode1,eIO9,true);
    }
  public void testCreateLink10() {
        lastTestCase=getCurrentTestMethodName();
        testLinkPlace(workPkg10,componentDiagramName10, fromType10, toType10,element10,elementName10,elementType10,treeNode1,eIO10,false);
        testLinkPlace(workPkg10,componentDiagramName10, fromType10, toType10,element10,elementName10,elementType10,treeNode1,eIO10,true);
    }
  public void testCreateLink11() {
        lastTestCase=getCurrentTestMethodName();
        testLinkPlace(workPkg11,componentDiagramName11, fromType11, toType11,element11,elementName11,elementType11,treeNode1,eIO11,false);
        testLinkPlace(workPkg11,componentDiagramName11, fromType11, toType11,element11,elementName11,elementType11,treeNode1,eIO11,true);
    }
  ...
  /*public void testCreateLink15() {
   ...
   private void testLinkPlace(String workPkg,String diagramName,ElementTypes fromType,ElementTypes toType,String element,String elementName,LinkTypes elementType,String treeNode,boolean useIO,boolean second)
   {
       log("---START, SECOND: "+second);
       String localElName1="El1";
       String localElName2="El2";
       String newName="LinkName"+counter++;
       //
       ...
       //
        ...
        if(second)
        {
            ...
            dE1=new DiagramElementOperator(d,localElName1);
            dE2=new DiagramElementOperator(d,localElName2);
        }
        else
        {
        ...
        drAr.clickMouse(dE1.getCenterPoint().x+(second?50:-50),(dE1.getCenterPoint().y+dE2.getCenterPoint().y)/2,1);
        ...
        LinkTypes elementTypeInProjectTree = null;
        try
        {
            for(int i=0;i<20 && testedlink==null;i++)
            {
                ...
                */
                elementTypeInProjectTree=elementType;
                ...
                testedlink=LinkOperator.findLink(dE1, dE2,new LinkOperator.LinkByTypeChooser(elementTypeInProjectTree),second?1:0);
        ...
        //
        if(second && (LinkTypes.IMPLEMENTATION.equals(elementType) || LinkTypes.GENERALIZATION.equals(elementType)))
        {
            assertTrue("Second "+LinkTypes.IMPLEMENTATION+" appears on diagram.",testedlink==null);
            return;
        }
        else
        {   
            assertTrue("Can't find link number "+(second?"2":"1"),testedlink!=null);
            assertTrue("There should be "+(second?"2":"1")+" links, now :"+dE1.getLinks().size()+" - "+dE2.getLinks().size(), dE1.getLinks().size()==(second?2:1) && dE2.getLinks().size()==(second?2:1));
        }
        //correct link if second goes with index 0
        log("NEED CORRECTION "+testedlink.getSource()+"? NAME: "+testedlink.getName());
        if(second && !(testedlink.getName()==null || testedlink.getName().equals("") || testedlink.getName().equals(defaultNewElementName)))
        {
            testedlink=LinkOperator.findLink(dE1, dE2,new LinkOperator.LinkByTypeChooser(elementType),0);
            log("CORRECTION: "+testedlink.getSource()+':'+testedlink.getName());
        }
        //
        testedlink.select();
        //
        ...
        //
      if(!LinkTypes.DERIVATION_EDGE.equals(elementType)) 
      {
          boolean good_name=newName.equals(testedlink.getName());
          log("LINK NAME:"+testedlink.getName());
          log("GOOD/SECOND:"+good_name+"/"+second);
          if(!good_name && second)
          {
              //try to find 0th link
                try
                {
                        testedlink=LinkOperator.findLink(dE1, dE2,new LinkOperator.LinkByTypeChooser(elementType),0);
                }
                ...
                if(testedlink!=null)
                {
                    good_name=newName.equals(testedlink.getName());
                    log("LINK 2ND ATTEMPT NAME:"+testedlink.getName());
                    log("2ND ATTEMPT SECOND/GOOD:"+good_name+"/"+second);
               }
          }
          if(!good_name)
          {
              log("start more invesigation");
              //TBD need more investigations
              String res="Num Links Fo 1st: "+dE1.getLinks().size()+", out: "+dE1.getOutLinks().size()+", out: "+dE1.getInLinks().size();
              log("res1");
              {
                  Iterator<LinkOperator> it=dE1.getLinks().iterator();
                  for(LinkOperator tmp=null;;)
                  {
                      if(it.hasNext())tmp=it.next();
                      else break;
                      res+="\nlink_1: "+tmp.getName()+":"+tmp.getType()+";";
                  }
              }
              res+="\nNum Links Fo 2nd: "+dE2.getLinks().size()+", out: "+dE2.getOutLinks().size()+", out: "+dE2.getInLinks().size();
              {
                  Iterator<LinkOperator> it=dE2.getLinks().iterator();
                  for(LinkOperator tmp=null;;)
                  {
                      if(it.hasNext())tmp=it.next();
                      else break;
                      res+="\nlink_2: "+tmp.getName()+":"+tmp.getType()+";";
                  }
              }
              log("end more invesigation");
              //*
              fail("Link name isn't correct: "+testedlink.getName()+", should be: "+newName+"//\n"+res);
          }
      }
        log("start nodes check");
         //
         ...
         if(useIO)
         {
             fail("unsupported testcase");
         }
         else
         {
             assertTrue("There should be "+newName+" node within Relationships node of "+localElName1,rel1.isChildPresent(newName));
             assertTrue("There should be "+newName+" node within Relationships node of "+localElName2,rel2.isChildPresent(newName));
             if(second)
             {
                 assertTrue("There should be second "+newName+" node within Relationships node of "+localElName1,rel1.isChildPresent("LinkName"+(counter-1)));
                 assertTrue("There should be second "+newName+" node within Relationships node of "+localElName2,rel2.isChildPresent("LinkName"+(counter-1)));
             }
