...
public class OperandsMoveProvider extends VerticalWithLimitMoveProvider {
    ...
    public void movementStarted(Widget widget) {
        ...
        int yMin=30;
        int yMax=widget.getParentWidget().getBounds().height-5;//TBD unhardcore 50??
        if(widget instanceof InteractionOperandWidget)
        {
            InteractionOperandWidget ioW=(InteractionOperandWidget) widget;
            Widget container=ioW.getParentWidget();
            for(Widget i:container.getChildren())//TBD Can be optimized if consider operands ordered always 
            {
                if(i==ioW)continue;
                int y=i.getLocation().y;
                if(y<ioW.getLocation().y)
                {
                    //one of previous
