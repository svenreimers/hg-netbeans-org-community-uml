 ...
 *
 * Created on April 8, 2005, 7:35 AM
 */
...
public class JavaEnumerationChangeHandler  extends JavaChangeHandler
{
   ...
   /** Creates a new instance of JavaEnumerationChangeHandler */
   public JavaEnumerationChangeHandler()
   {
   }
   
   public JavaEnumerationChangeHandler(JavaChangeHandlerUtilities utilities)
    {
    	super(utilities);
    }
    
	public JavaEnumerationChangeHandler(JavaChangeHandler copy )
	{
		super(copy);
	}
   ...
   public void handleRequest(IRequestValidator requestValidator)
	{
		...
		{	
			if (m_Utilities != null)
			{
				...
				if ( details != null && 
					 details.rtElementKind == RTElementKind.RCT_ENUMERATION )
				{
   ...
   protected void transform( IRequestValidator requestValidator, int cType, int cDetail )
	...
	public void transform(IRequestValidator requestValidator, IClassifier pClass)
	{
		if (pClass != null)
		{
			...
			// A classifier is being transformed into an enumeration.
			// We must create literals, create all attributes.
			// The class and all of its operations will as is,
			...
			if (attributes != null)
			{
				...
				if (iter != null)
				{
					while (iter.hasNext())
					{
					   ...
					   if (attr != null)
					   {
							attrhandler.transformToEnumeration(attr, pClass);
					   }
					}
				}			
			}
				...
				if (iter != null)
				{
					while (iter.hasNext())
					{
					   ...
					   if (oper != null)
					   {
							...
							opHandler.transformToEnumeration(requestValidator, oper, pClass, noAbstractProcessing);
					   }
					}
				}			
			}
				...
				if (iter != null)
				{
					while (iter.hasNext())
					{
					   ...
					   if (end != null && end instanceof IAttribute)
					   {
							...
							attrhandler.transformToEnumeration ( pEnd, pClass );
					   }
					}
				}			
			}
