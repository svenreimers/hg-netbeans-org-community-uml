	...
	public static void openURL(String url) throws IOException {
		...
		switch (jvm) {
			...
			case OTHER:
				...
				try
				{
					ETSystem.out.println("Trying to launch " + url + " with " + browser);
					...
					try 
					{
						...
						{	
							...
							ETSystem.out.println("2nd try to launch " + url + " with " + browser);
				...
				catch (Exception e)
				{
					try
					{
						// failed to launch Netscape, so now try Mozilla
						ETSystem.out.println("Trying to launch " + url + " with mozilla");
						process = Runtime.getRuntime().exec(new String[] { "mozilla ",
															NETSCAPE_REMOTE_PARAMETER,
															NETSCAPE_OPEN_PARAMETER_START +
															url +
															NETSCAPE_OPEN_PARAMETER_END });
						...
						try 
						{
							...
							if (exitCode != 0) 
							{	
								// if Mozilla was not open
								ETSystem.out.println("2nd Try to launch " + url + " with mozilla");
					...
					{
						// if Mozilla was not open
						ETSystem.out.println("3rd Try to launch " + url + " with mozilla");
						process = Runtime.getRuntime().exec(new String[] { "mozilla ", url });
					}
				}
				break;
